...
func runMount(cmd *cobra.Command, args []string) error {
	...
	server, err := fs.Mount(config.MountPath, filterFS, opts)
	if err != nil {
...
func runUnmount(cmd *cobra.Command, args []string) error {
	...
	if err := tryFusermount(mountPath); err == nil {
		logger.Info("Successfully unmounted with fusermount")
		return nil
	}
	
	// Try umount as fallback
	if err := tryUmount(mountPath); err == nil {
		logger.Info("Successfully unmounted with umount") 
		return nil
	}
	
	return fmt.Errorf("failed to unmount %s: try running 'fusermount -u %s' or 'umount %s' manually", mountPath, mountPath, mountPath)
}

func tryFusermount(mountPath string) error {
	cmd := exec.Command("fusermount", "-u", mountPath)
	return cmd.Run()
}

func tryUmount(mountPath string) error {
